...
def save_zones_to_mongo(dataframe, db_name, collection_name):
    """
...
class TestSaveZonesToMongo(unittest.TestCase):

    def setUp(self):
        # Create a sample DataFrame with zone names and IDs
        data = {
            'ext_id': [1, 2, 3],
            'name': ['North Gondar', 'East Wellega', 'Borena']
        }
    ...
    def test_save_zones_to_mongo(self):
        ...
        result = save_zones_to_mongo(self.df, db_name, collection_name)
