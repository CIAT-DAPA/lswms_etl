...
#checks the current data from mongodb and returns filterd  dataframe from postgreSQL db df to be imported to the monogod 
#which potentially limits the importing process to be limited to certain rows
def filter_import(df):
    d=Waterpoint.objects()
    list_dfs=[]
    init_day=datetime.today().date()-timedelta(days=57)
    lag_day=datetime(2022,1,1).date()
    
    while(init_day>lag_day):
        wp_list=[]
        for i in range(len(d)):
            #start with lag2
            if Monitored.objects(date= init_day , waterpoint= d[i].id):
                pass

            else:
                # print(d[i].ext_id)
                wp_list.append(int(d[i].ext_id))
        if len(wp_list)==0:
            break
        # print(wp_list,init_day)
        df_temp=df[(df.date==init_day) & (df.location_id.isin(wp_list))]
        init_day=init_day-timedelta(days=1)
        list_dfs.append(df_temp)
        #pd.concat([df_filterd, dd], ignore_index=True, sort=False)
    df_filterd=pd.concat(list_dfs).reset_index() 
    return df_filterd
...
df_filterd=filter_import(df)
etl_monitored(df_filterd)
